...
function generateGroupsSchedulers() {
  console.log(version);
  promos = [];
  let But3_done = false;
...
async function nextEventFilter(event) {
...
onMounted(() => {
  ...
  getAllPlannings();
  const delay = 1000 * 60 * 5; // Refresh toutes les 5 minutes
  refreshInterval = setInterval(getAllPlannings, delay);
});
